...
**`origin`에 code를 push할 때는 항상 브랜치를 만들어 PR을 올리고, 사용자의 허락을 받은 뒤에 push한다.**
로컬 커밋(및 코드 리뷰까지)은 허락 없이 바로 진행해도 되지만, `git push`로 원격 저장소에 반영하는 시점
직전에는 반드시 먼저 확인받는다. `main`에 직접 push하지 않고 별도 브랜치 + PR 흐름을 기본으로 한다.
