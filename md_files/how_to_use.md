...
실행된다 (단, Windows 전용 기능이라 다른 OS에서는 동작하지 않는다).

- **시스템 정보**: Windows 버전(빌드 번호 포함) 다음 줄에 **CPU 모델명**, 그 아래 **CPU 사용률**과
  **메모리 사용률**이 막대그래프 + 퍼센트/용량 텍스트로 8초마다 자동 갱신됨. 라벨/막대/수치 컬럼이
  모두 같은 위치에서 시작하도록 격자로 정렬되어 있음 (디스크 줄들과도 동일한 정렬). 이 8초 주기
  갱신(디스크 포함)은 **"윈도우 현황" 탭이 실제로 선택되어 화면에 보이는 동안에만** 돈다 — 다른
  탭을 보고 있을 때는 멈춰서 불필요하게 CPU를 쓰지 않고, 탭으로 돌아오면 즉시 한 번 새로고침한 뒤
  다시 8초 주기로 갱신된다.
  - 사용률이 **90% 이상이면 막대그래프가 빨간색으로 바뀜** (CPU/메모리/디스크 공통)
- **디스크**: **C/D/E는 실제로 없어도 항상 세 줄이 표시됨** (없는 드라이브는 "없음"으로 표시,
  화면 구성이 매번 흔들리지 않도록). C/D/E 외에 다른 고정 드라이브가 더 있으면 그 뒤에 이어서
  표시됨(이동식/광학/네트워크 드라이브는 제외). 각 줄은 드라이브 문자 + 막대그래프 +
  "사용량/전체 용량 (퍼센트)"가 3초마다 갱신됨
- **상위 프로세스** 버튼 — CPU/메모리 사용량 상위 5개 프로세스를 각각 표(이름/PID/사용량)로 보여주는
  창이 뜸 (클릭 시점에 짧게(0.3초) 두 번 측정해서 CPU %를 계산하므로 살짝 지연 있음)
- **폴더 용량** 버튼 — Temp 폴더/다운로드 폴더의 전체 용량을 계산해서 보여줌 (폴더가 크면 계산에
  몇 초 걸릴 수 있음)
- **시작 프로그램 목록** 버튼 — 레지스트리(`HKCU`/`HKLM`의 `...\Run`)와 시작프로그램 폴더에 등록된
  항목을 이름/실행 명령·경로/출처 표로 보여줌
- **시스템 변수 바로보기** 버튼 — Windows 자체의 "환경 변수" 편집 창을 바로 띄움 (`rundll32
