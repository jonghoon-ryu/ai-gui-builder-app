...
- **폴더 용량** — Temp 폴더/다운로드 폴더의 전체 용량을 계산해서 보여줌 (폴더가 크면 계산에
  몇 초 걸릴 수 있음)
...
- **temp 파일**: 폴더 아이콘 옆에 "N개 파일, 용량"이 표시됨 (`%TEMP%` 폴더를 재귀적으로 훑은 합계,
  탭에 들어올 때/탭 안에서 새로고침될 때 계산되며 폴더가 크면 잠깐 걸릴 수 있음 — "폴더 용량"의
  Temp 항목과 같은 방식으로 계산하지만 이쪽은 상시 표시됨)
...
갱신될 때마다 값이 조금씩 변한다 (측정 방식상 정상). "상위 프로세스"/"폴더 용량"은 클릭했을 때만
계산하는 버튼식이라(8초 주기 자동 갱신에는 포함 안 됨) 부하가 크지 않다. standalone으로 내보내면
