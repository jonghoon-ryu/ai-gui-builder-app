...
2. 캔버스에 놓인 위젯을 **우클릭**하면 메뉴가 뜬다:
   ...
   - **선택한 사각형들 정렬** — 템플릿 사각형(컨테이너) 2개 이상을 선택(러버밴드)한 뒤 그 중 하나를
     우클릭하면 뜸; 선택된 사각형들의 좌우/상하 여백을 서로 또는 캔버스 가장자리와 맞춰 자동 정렬함
     (자식들이 차지한 공간보다 작아지는 사각형은 위치만 옮기고 크기는 그대로 둠)
   - **내부 위젯 정렬** — 자식 위젯이 2개 이상 들어있는 템플릿 사각형을 우클릭하면 뜸; 그 사각형
     안의 자식들끼리 좌우/상하 여백을 맞춰 자동 정렬함
