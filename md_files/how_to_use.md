...
- **시스템 정보**: Windows 버전(빌드 번호 포함) 다음 줄에 **CPU 모델명**, 그 아래 **CPU 사용률**과
  **메모리 사용률**이 막대그래프 + 퍼센트/용량 텍스트로 3초마다 자동 갱신됨. 라벨/막대/수치 컬럼이
  모두 같은 위치에서 시작하도록 격자로 정렬되어 있음 (디스크 줄들과도 동일한 정렬)
...
계산하는 버튼식이라(3초 주기 자동 갱신에는 포함 안 됨) 부하가 크지 않다. standalone으로 내보내면
