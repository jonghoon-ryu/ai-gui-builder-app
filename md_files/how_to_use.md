...
- **local drive 검색** 버튼 — 누르면 C/D/E 드라이브를 처음부터 훑어서 `.git` 폴더가 있는 디렉토리를
  찾아 **6개 상자의 local 칸을 순서대로 채움** (기존에 입력되어 있던 값은 덮어씀). 최대 6개까지만
  찾고 그 이상은 찾지 않음 (더 못 찾으면 있는 만큼만 채우고 몇 개 찾았는지 안내). remote 칸은 건드리지
  않으므로 비교하려면 직접 입력해야 함. 드라이브 전체 검색이라 시간이 걸릴 수 있어서 별도 스레드에서
  실행되며(화면이 멈추지 않음), 검색 중에는 버튼이 "검색 중..."으로 바뀌고 비활성화됨
- **전체 status check** 버튼 — 누르면 6개 상자 중 remote/local이 **둘 다 입력된** 상자를 모두 찾아
  한꺼번에 비교를 실행해서, 각 상자의 "비교결과" 버튼 색을 갱신함. 여러 개를 한 번에 확인할 때
  하나하나 "비교"를 누를 필요 없이 이 버튼 한 번으로 끝난다. 네트워크 조회가 여러 번 걸릴 수 있어서
  별도 스레드에서 실행되며(화면이 멈추지 않음), 확인 중에는 버튼이 "확인 중..."으로 바뀌고
  비활성화됨. 입력된 상자가 하나도 없으면 안내 팝업만 뜸
