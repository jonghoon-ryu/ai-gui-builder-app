...
class GeneratedApp(QMainWindow):
    def __init__(self):
        ...
        self.tab1_urlbox_1.setText("https://ko.wikipedia.org/wiki/%EC%86%94%EB%A6%AC%EB%93%9C_%EC%8A%A4%ED%85%8C%EC%9D%B4%ED%8A%B8_%EB%93%9C%EB%9D%BC%EC%9D%B4%EB%B8%8C")
        ...
        self.tab1_urlbox_1.move(159, 166)
        ...
        self.tab1_dirbox_1.setText("/mnt/0C084C3768880E8A/RyuVault/wiki/ssd")
    ...
    def _tab1_button_1_on_event(self):
        url = self.tab1_urlbox_1.text()
        save_dir = self.tab1_dirbox_1.text()
        make_dir(save_dir)
        html = fetch_url(url)
        md = html_to_markdown(html)
        md_path = save_dir + "/output.md"
        write_file(md_path, md)
        has_images = "![" in md
        if has_images:
            if self.tab1_radiobutton_1.isChecked():
                QMessageBox.information(self, "완료", "문서를 마크다운으로 변환하여 저장했습니다.\n이미지 파일을 자동으로 분류하여 저장합니다.")
            elif self.tab1_radiobutton_2.isChecked():
                QMessageBox.information(self, "완료", "문서를 마크다운으로 변환하여 저장했습니다.\nClaude에게 요청하여 이미지 파일을 분류하여 저장합니다.")
            else:
                QMessageBox.information(self, "완료", "문서를 마크다운으로 변환하여 저장했습니다.\n이미지 파일이 있으나 분류 방식이 선택되지 않았습니다.")
        else:
            QMessageBox.information(self, "완료", "문서를 마크다운으로 변환하여 저장했습니다.")
